#!/usr/bin/env python3
"""
DPO training scaffold for collab-eval row-preservation policy shaping.

Backend: mlx-lm-lora (mlx_lm_lora.train --train-mode dpo).
Install: pip install -U mlx-lm-lora
Tested against mlx-lm-lora v2.1.0.

Adapter-stacking strategy: SFT v3 adapter is fused into the base model first
(see scripts/fuse_v3_sft.py -- requires --dequantize for 4-bit base models).
The fused model is used as both --model and --reference-model-path,
anchoring KL at SFT v3 behavior.

Usage:
  python collab-eval/scripts/train_collab_dpo.py \\
      --config collab-eval/configs/dpo_collab_eval_qwen25_3b.yaml
  python collab-eval/scripts/train_collab_dpo.py \\
      --config collab-eval/configs/dpo_collab_eval_qwen25_3b.yaml --dry-run
  python collab-eval/scripts/train_collab_dpo.py --check-only
"""
...
def build_dpo_command(
    ...
    if train_cfg.get("grad_checkpoint", False):
...
def main() -> None:
    ...
    print(f"\ncollab-eval DPO training scaffold")
    ...
    if not available:
        print(
            ...
            "  Then: python collab-eval/scripts/fuse_v3_sft.py  # fuse SFT v3 before DPO\n"
        )
        sys.exit(1)
    ...
    try:
        if args.dry_run:
            ...
            import json
            print(
                f"\n  LoRA params (passed via -c temp YAML, "
                f"mlx-lm-lora v2.1.0 has no --lora-parameters flag):"
            )
            print(f"    {json.dumps(lora_params)}")
            grad = config["training"].get("grad_checkpoint", False)
            print(f"\n  grad_checkpoint: {grad}")
            print()
            sys.exit(0)
