...
Each task type has one hardcoded example. A real training setup would generate a
distribution of tasks: different documents, different seeded problems, different
constraint sets. This is straightforward for the spreadsheet task (parameterize the
number of rows, unit mixing ratio, blank-row count) and harder for document revision
(requires a template library or a generator model).
...
### 6.2 Human-in-the-loop grading budget

The harness's judgment dimensions (faithfulness, citation accuracy) are cheap to score
with an LLM judge but will be wrong some fraction of the time. A production RL setup
would route a sample of episodes to human raters to calibrate the judge and catch
systematic errors. The grading budget decision — what fraction of episodes need human
review — is a cost-quality trade-off that this harness does not address.
